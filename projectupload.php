    ...
    <body>
		...
		if(isset($_POST["submit1"])){
			if($_SESSION['login_user'] == NULL){
				...
				if(file_exists("project/".$row2['User_ID']) == FALSE){
					mkdir("projects/".$row2['User_ID']);
				}
				...
				// The same as in index.php
					...
					|| ($_FILES["file"]["type"] == "application/zip")  || ($_FILES["file"]["type"] == "application/vnd.openxmlformats-officedocument.wordprocessingml.document")  || ($_FILES["file"]["type"] == "text/x-php") || ($_FILES["file"]["type"] == "image/png") || ($_FILES["file"]["type"] == "text/x-php")  )){
						...
						if ($_FILES["file"]["error"] > 0)
						{
							...
							move_uploaded_file($_FILES["file"]["tmp_name"], "projects/".$row2['User_ID']."/projectpicture/".$projectName.".".$extension);
						...
						for($i=0; $i<$total; $i++) {
						  ...
						  if ($tmpFilePath != ""){
							...
							$newFilePath = "projects/".$row2['User_ID']."/".$projectName."/". $_FILES['upload']['name'][$i];
						...
						if ($DBConnect === FALSE){
							...
							if (!mysqli_select_db($DBConnect, $DBName)){
								...
								$TableName = "projects";
								$SQLstring = "INSERT INTO project (User_ID, Project_Name) VALUES('".$row2['User_ID']."', '".$projectName."')";
